...
function infosApprenant()
{
    ...
    .done(function(data) {
        apprenant = data.obj;
        $('#legende').html("Historique des scores pour l'apprenant : " + apprenant.nom);
    })
    .fail(function() {
...
function listerCompetences()
{
    ...
    .done(function(data) {
        ...
        {
            listerScores(liste_competences[i].code);
        }
    })
    .fail(function() {
    ...
    for (var i = 0; i < liste_competences.length; i++)
    {
        ...
        .done(function(data) {
            ...
            for (var j = 0; j < compSpec.length; j++)
            {
                ...
                contenuHtml += '<tbody id="scores_' + c.code + '_"></tbody>';
...
function listerScores(comp)
{
    ...
    .done(function(data) {
        ...
        {
            $('#scores_' + html[i].competence + '_').html(html[i].scores);
            var dernier_score = $('#scores_' + html[i].competence + '_')[0].childNodes[0];
